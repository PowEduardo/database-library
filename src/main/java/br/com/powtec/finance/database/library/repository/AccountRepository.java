...
public interface AccountRepository extends JpaRepository<AccountModel, Long>, JpaSpecificationExecutor<AccountModel> {
  ...
  @Query(value = "SELECT SUM(CASE WHEN movements.type = 'DEBIT' THEN movements.value * -1.0 ELSE movements.value END) FROM MovementModel AS movements JOIN movements.account AS account WHERE account.id = :accountId")
